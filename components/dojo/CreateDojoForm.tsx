...
import { Dojo, User, MartialArt } from '../../types';
import { MARTIAL_ARTS } from '../../constants';

interface CreateDojoFormProps {
  onDojoCreated: (dojo: Dojo) => void;
  currentUser: User;
}

const CreateDojoForm: React.FC<CreateDojoFormProps> = ({ onDojoCreated, currentUser }) => {
  const [name, setName] = useState('');
  const [teamName, setTeamName] = useState('');
  const [selectedModalities, setSelectedModalities] = useState<MartialArt[]>([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !teamName || selectedModalities.length === 0) {
      alert('Por favor, preencha todos os campos e selecione ao menos uma modalidade.');
      return;
    }
    // FIX: Add missing `exams` and `graduationEvents` properties to conform to the Dojo type.
    const newDojo: Dojo = {
    ...
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input id="teamName" type="text" value={teamName} onChange={(e) => setTeamName(e.target.value)} required className="bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-red-500 dark:focus:ring-amber-500 focus:border-red-500 dark:focus:border-amber-500 block w-full p-2.5" placeholder="Ex: Checkmat" />
        </div>
        <div>
