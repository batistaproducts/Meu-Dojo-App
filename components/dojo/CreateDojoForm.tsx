  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-lg shadow-2xl animate-fade-in">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold font-cinzel text-gray-900 dark:text-white">Crie seu Dojo</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input id="dojoName" type="text" value={name} onChange={(e) => setName(e.target.value)} required className="bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-red-500 dark:focus:ring-white focus:border-red-500 dark:focus:border-white block w-full p-2.5" placeholder="Ex: Academia Feras do Tatame" />
        </div>
        <div>
          <label htmlFor="teamName" className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Nome da Equipe</label>
          <input id="teamName" type="text" value={team_name} onChange={(e) => setTeamName(e.target.value)} required className="bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-red-500 dark:focus:ring-white focus:border-red-500 dark:focus:border-white block w-full p-2.5" placeholder="Ex: Checkmat" />
        </div>
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {MARTIAL_ARTS.map(art => (
              <button
                ...
                className={`p-3 text-sm text-center rounded-lg border-2 transition-colors ${modalities.some(m => m.name === art.name) ? 'bg-red-600 border-red-500 text-white' : 'bg-gray-200 dark:bg-gray-700 border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'}`}
              >
                {art.name}
              </button>
            ))}
        ...
        <button type="submit" disabled={loading} className="w-full text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-400 font-bold rounded-lg text-lg px-5 py-3 text-center transition-colors duration-300 disabled:opacity-50">
