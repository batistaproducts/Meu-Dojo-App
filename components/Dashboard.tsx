...
const Card: React.FC<DashboardCardProps> = ({ view, title, description, icon, onClick }) => (
    <div
        ...
        className="bg-white dark:bg-gray-800 rounded-lg p-6 md:p-8 shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-red-500/20 dark:hover:shadow-amber-500/20 cursor-pointer group flex flex-col items-center"
    >
        {icon}
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white group-hover:text-red-700 dark:group-hover:text-amber-400 transition-colors duration-300 mb-2">{title}</h3>
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, permissions }) => {
    ...
    const availableCards: Omit<DashboardCardProps, 'onClick'>[] = [
        ...
        },
        {
            ...
            icon: <MedalIcon className="h-16 w-16 mb-4 text-red-700 dark:text-amber-400 group-hover:text-red-600 dark:group-hover:text-amber-300 transition-colors" />
        }
    ];
